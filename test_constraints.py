...
class BeowulfTestConstraints:
    ...
    def test_no_extra_fields(self):
        """Each line should not have extra fields beyond 'line', 'OE', and 'ME'."""
        allowed_fields = {'line', 'OE', 'ME'}
        for i, line_data in enumerate(self.lines):
            extra = set(line_data.keys()) - allowed_fields
            assert not extra, f"Line {i} has extra fields: {extra}"
        return True

    def test_no_empty_lines(self):
        """No line except line 0 should have both OE and ME empty."""
        for i, line_data in enumerate(self.lines):
            if i == 0:
                continue
            assert line_data['OE'] or line_data['ME'], f"Line {i} has both OE and ME empty"
        return True

    def test_fitt_boundaries_do_not_overlap(self):
        """Fitt boundaries should not overlap (except for adjacent boundaries)."""
        previous_end = None
        for i, (start, end, name) in enumerate(FITT_BOUNDARIES):
            if i == 24:  # Skip non-existent fitt 24
                continue
            if previous_end is not None:
                assert start == previous_end + 1, f"Fitt {i} starts at {start}, expected {previous_end + 1}"
            previous_end = end
        return True
